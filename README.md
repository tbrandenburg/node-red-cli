...
This repository demonstrates a host-side adapter for Node-RED 5.0.4. The
adapter invokes an existing `link in` node and captures the response from a
`link out` node in return mode as a Promise.

The included test flow computes `x + y`:
...
4. ✅ An unchanged SHA-256 hash of `flows.json` before and after the call.
...
**Proof of concept:** the approach works for the included Node-RED 5.0.4 test
flow. The link-call internals used here are not stabilized as a public
Node-RED API. Production use therefore requires deliberate version pinning,
integration tests, and robust error handling for ambiguous or non-terminating
flows.
